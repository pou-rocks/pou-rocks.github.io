(function () {
  ...
  var RTL = ['ar']
  var locale = 'en', data = null, spec = null, state = null
  ...
  var SPECS = {
    'precision-parts': {
      ...
      levels: function () { return { from: range(0, 9), to: range(1, 10), label: function (i) { return 'i' + i + '  ·  Lv.' + (30 + 5 * i) } } },
      cost: function (d, id, from, to) {
        var b = d.buildings.filter(function (x) { return x.id === id })[0]
        var steps = b.bands.filter(function (x) { return x.band > from && x.band <= to }).map(function (x) {
          return { label: 'i' + (x.band - 1) + '→i' + x.band, detail: 'Lv.' + x.levels, per: x.perLevel, amounts: { parts: x.total } }
        })
        return steps
      }
    },
    ...
    'hero-weapon': {
      ...
      entityLabel: 'Weapon',
      materials: function () { return [{ key: 'frag', name: 'Fragments' }] },
      entities: function (d) {
    ...
    'hero-stars': {
      ...
      materials: function () { return [{ key: 'frag', name: 'Fragments' }] },
      entities: null,
      levels: function () { return { from: range(0, 4), to: range(1, 5), label: function (i) { return i + '★' } } },
      cost: function (d, _id, from, to) {
        return d.bands.filter(function (b) { return b.from >= from && b.to <= to }).map(function (b) {
          return { label: b.from + '★→' + b.to + '★', detail: b.tickCosts.join(' + '), per: null, amounts: { frag: b.total } }
        })
      }
    },
    'hero-equipment': {
      ...
      entities: function (d) {
        var out = [{ value: 'promote', label: 'Promotion (rank 0–36)', desc: 'Power Cores + Boost Ore + DX-Blueprints' }]
        d.upgrade.curves.forEach(function (c) {
          out.push({ value: 'upgrade-' + c.quality, label: 'Levelling · quality ' + c.quality + ' (0–' + c.maxLevel + ')', desc: 'Boost Ore only' })
        })
        return out
      },
      levelsFor: function (d, id) {
        if (id === 'promote') return { from: range(0, 35), to: range(1, 36), label: function (i) { return 'Rank ' + i } }
        var q = +id.split('-')[1]
    ...
    'vehicle-chips': {
      ...
      entityLabel: 'Chip',
      materials: function () { return [{ key: 'dup', name: 'Duplicate chips' }] },
      entities: function (d) {
      ...
      levels: function () { return { from: range(0, 9), to: range(1, 10), label: function (i) { return i + '★' } } },
      cost: function (d, id, from, to) {
        ...
        return lad.stars.filter(function (s) { return s.from >= from && s.to <= to }).map(function (s) {
          return { label: s.from + '★→' + s.to + '★', detail: s.exp ? s.exp.toLocaleString() + ' chip EXP' : '', per: null, amounts: { dup: s.chips } }
        })
      }
    }
  }
  ...
  function render () {
    ...
    if (!state.rows.length) {
      ...
      e.textContent = 'Nothing selected yet — add a row to start.'
      host.appendChild(e)
    }
    var entities = ents()
    state.rows.forEach(function (r, idx) {
      ...
      if (entities) {
        ...
        t.className = 'flex-1 text-sm font-semibold py-3'
        t.textContent = spec.title.replace(' Calculator', '')
        head.appendChild(t)
      }
      if (entities) {
      ...
      fw.innerHTML = '<label class="block text-xs uppercase tracking-wide text-theme-muted mb-1">From</label>'
      fw.appendChild(sel('w-full bg-dark-card border border-dark-border rounded-md px-2 py-3 text-sm',
        ...
        function (v) { r.from = +v; if (r.to <= r.from) r.to = l.to.filter(function (x) { return x > r.from })[0]; save(); render() }, 'From'))
      var tw = document.createElement('div')
      tw.innerHTML = '<label class="block text-xs uppercase tracking-wide text-theme-muted mb-1">To</label>'
      tw.appendChild(sel('w-full bg-dark-card border border-dark-border rounded-md px-2 py-3 text-sm',
        ...
        function (v) { r.to = +v; save(); render() }, 'To'))
      ...
      foot.innerHTML = '<span class="text-xs text-theme-muted">' + st.length + (st.length === 1 ? ' step' : ' steps') + '</span>' +
  ...
  function renderTotals () {
    var t = totals(), mats = spec.materials(data)
    ...
    mats.forEach(function (m) {
      ...
      row.innerHTML = '<span class="text-xs text-theme-muted truncate">' + m.name + '</span>' +
        '<span class="text-xl font-bold text-accent tabular">' + fmt(t[m.key] || 0) + '</span>'
      box.appendChild(row)
    })
    ...
    mats.forEach(function (m) {
      ...
      var need = t[m.key] || 0, got = parseInt(state.have[m.key] || '0', 10) || 0
      if (!state.have[m.key]) { out.textContent = ''; }
      else if (got >= need) { out.textContent = 'Covered'; out.classList.add('text-green-400') }
      else { out.textContent = 'Short ' + fmt(need - got); out.classList.add('text-accent') }
  ...
  function renderBreakdown () {
    ...
    var html = '<table class="w-full text-sm border-collapse"><thead><tr class="text-theme-muted text-xs uppercase tracking-wide">' +
      (entities ? '<th class="text-start py-2 pe-3">' + (spec.entityLabel || '') + '</th>' : '') +
      '<th class="text-start py-2 pe-3">Step</th><th class="text-start py-2 pe-3">Detail</th>' +
    ...
    html += '</tbody><tfoot><tr class="border-t-2 border-dark-border">' +
      '<td class="py-2 font-semibold" colspan="' + (entities ? 3 : 2) + '">Total</td>' +
      mats.map(function (m) { return '<td class="py-2 pe-3 text-end font-bold text-accent tabular">' + fmt(t[m.key] || 0) + '</td>' }).join('') +
      '</tr></tfoot></table>'
    host.innerHTML = html
  }
  ...
  function boot () {
    ...
    document.title = spec.title + ' · DWS Planner'
    ...
    function applyOpen () {
      ...
      tb.querySelector('span').textContent = state.open ? 'Hide breakdown' : 'Show breakdown'
    ...
    document.getElementById('meta').textContent = 'Data extracted from Dark War Survival build ' + data.apkBuild +
      '. Config cost before buffs — in-game prices can be lower.'
    render()
  }
  ...
  fetch('/data/' + spec.data + '.json')
    .then(function (r) { if (!r.ok) throw new Error('http ' + r.status); return r.json() })
    .then(function (j) { data = j; boot() })
    .catch(function () {
      document.getElementById('rows').innerHTML = '<div class="glass-card text-sm text-theme-muted">Could not load the data. Try reloading.</div>'
    })
})()
